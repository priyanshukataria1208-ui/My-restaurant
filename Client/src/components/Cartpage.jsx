  ...
  const applyCoupon = async (coupon) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setCart(res.data.cart);       // backend se updated cart
        setDiscount(res.data.cart.discount || 0);  // backend discount
              ...
              <div className="space-y-3" >
                {availableCoupons.map((c) => {
                  ...
                  return (
                    <div
                      key={c.code}
                      className={`p-3 mt-4 rounded-lg ${eligible ? "bg-green-700" : "bg-slate-700"
                        }`}
                    >
